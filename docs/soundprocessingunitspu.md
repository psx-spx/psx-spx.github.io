...
The PlayStation's SPU is a direct evolution of the
[S-DSP](https://problemkaputt.de/fullsnes.htm#snesaudioprocessingunitapu) chip
Sony designed for the SNES. The main differences are:

- the SPU is no longer driven by a dedicated sound CPU but still has its own
  RAM bank for samples, upgraded from 64KB to 512KB;
- the number of voices and output sample rate were bumped to 24 and 44100Hz
  respectively;
- the BRR ADPCM format was extended from 9-byte blocks to 16-byte ones, with 5
  filters instead of 4;
- the S-DSP's "echo" functionality was replaced with a full stereo Schroeder
  reverberator;
...
```
Sony did not release register-level PS1 SPU documentation, but most register
names can be inferred from their SNES and/or PS2 counterparts (the PS2's SPU2 is
basically two PS1 SPU "cores" sharing the same memory, with minor changes). Some
aspects such as reverb were never documented at all, so the reverb configuration
registers listed here are given unofficial names instead.<br/>
...
##   SPU ADPCM Samples
The SPU supports only "bit rate reduction" (BRR) ADPCM compressed samples
(uncompressed samples seem to be totally unsupported; leaving apart that one can
write uncompressed 16bit PCM samples to the Reverb Buffer, which can be then
output at 22050Hz, as long as they aren't overwritten by the hardware).<br/>

#### `0x1f801c06 + 0x10*N`: `SSA` (sample start address)
...
#### `0x1f801c0e + 0x10*N`: `LSAX` (loop start address)
...
The PSX supports two BRR formats: SPU-ADPCM (as described above), and XA-ADPCM.
XA-ADPCM is decompressed by the CDROM Controller, and sent directly to the sound
mixer, without needing to store the data in SPU RAM, nor needing to use a Voice
channel.<br/>
...
#### `0x1f801d92`: `PMON1` (voice 16..23 pitch modulation enable)
```
  0-7   Flags for Voice 16..23  (0=Normal, 1=Modulate by Voice 15..22)
  8-15  Not used
```
...
#### `0x1f801d80`: `MVOLL` (main volume left)
#### `0x1f801d82`: `MVOLR` (main volume right)
#### `0x1f801c00 + 0x10*N`: `VOLL` (voice volume left)
#### `0x1f801c02 + 0x10*N`: `VOLR` (voice volume right)
Direct Volume Mode (when Bit15=0):<br/>
```
  15    Must be zero      (0=Direct Mode)
...
#### `0x1f801db0`: `AVOLL` (I2SA/CD-ROM volume left)
#### `0x1f801db2`: `AVOLR` (I2SA/CD-ROM volume right)
#### `0x1f801db4`: `BVOLL` (I2SB/PIO volume left)
#### `0x1f801db6`: `BVOLR` (I2SB/PIO volume right)
```
  0-15  Volume  (-8000h..+7FFFh)
```
Note: The CD-ROM controller supports additional CD volume control (including
...
#### `0x1f801c0c + 0x10*N`: `ENVX` (voice envelope volume)
```
...
Step?!<br/>
This register is most commonly used to check if a given channel has finished
playback of a sample and is idle.<br/>

#### `0x1f801db8`: `MVOLXL` (current main volume left)
#### `0x1f801dba`: `MVOLXR` (current main volume right)
#### `0x1f801e00 + 0x4*N`: `VOLXL` (current voice volume left)
#### `0x1f801e02 + 0x4*N`: `VOLXR` (current voice volume right)
```
  0-15  Current Volume  (-8000h..+7FFFh)
```
These are internal registers, normally not used by software (the Volume
settings are usually set via Ports 1F801D80h and 1F801C00h+N\*10h). The final
volume applied to each voice is given by:<br/>
```
  Lvol = (ENVX * VOLXL) >> 15  ;-8000h..+7FFFh
  Rvol = (ENVX * VOLXR) >> 15  ;-8000h..+7FFFh
```
...
##   SPU Voice Flags
#### `0x1f801d88`: `KON0` (voice 0..15 key on, write-only)
```
  0-15  Voice 0..15 On  (0=No change, 1=Start Attack/Decay/Sustain)
```

#### `0x1f801d8a`: `KON1` (voice 16..23 key on, write-only)
```
  0-7   Voice 16..23 On  (0=No change, 1=Start Attack/Decay/Sustain)
  8-15  Not used
```
...
#### `0x1f801d9c`: `ENDX0` (voice 0..15 end reached, read-only)
```
  0-15  Voice 0..15 Status (0=Newly Keyed On, 1=Reached LOOP-END)
```
...
Some emulators do not implement this register.<br/>
...
##   SPU Noise Generator
#### `0x1f801d94`: `NON0` (voice 0..15 noise enable)
```
  0-15  Voice 0..15 Noise (0=ADPCM, 1=Noise)
```

#### `0x1f801d96`: `NON1` (voice 16..23 noise enable)
```
  0-7   Voice 16..23 Noise (0=ADPCM, 1=Noise)
  8-15  Not used
```
...
ATTR register (the ADPCM Sample Rate has absolutely no effect on noise), so
...
#### `0x1f801daa`: `ATTR` (common attribute register)
```
...
```
Changes to bit0-5 aren't applied immediately; after writing to ATTR, it'd be
usually recommended to wait until the LSBs of STATX are updated accordingly.
Before setting a new Transfer Mode, it'd be recommended first to set the "Stop"
mode (and, again, wait until Stop is applied in STATX).<br/>

#### `0x1f801dae`: `STATX` (status register, read-only)
```
  ...
  7     Data Transfer DMA Read/Write Request ;seems to be same as ATTR.Bit5
  6     IRQ9 Flag                        (0=No, 1=Interrupt Request)
  5-0   Current SPU Mode   (same as ATTR.Bit5-0, but, applied a bit delayed)
```
When switching ATTR to DMA-read mode, status bit9 and bit7 aren't set
...
The STATX register should be treated read-only (writing is possible in so far
...
##   SPU Memory Access
#### `0x1f801da6`: `TSA` (transfer start address)
```
...
```
  ...
  2-3   RAM chip size                (0=invalid?, 1=512KB, 2=2MB, 3=invalid?)
  4-15  Unknown
```
Possible values for bits 1-3 are:<br/>
```
  010 = 512KB total (512KB bank on /OE0)
  011 = 1MB total   (512KB bank on /OE0 + 512KB bank on /OE1)
  100 = 2MB total   (2MB bank on /OE0)
  101 = 4MB total   (2MB bank on /OE0 + 2MB bank on /OE1)
  others = invalid
```
This register is uninitialized on power-on and must be explicitly set to 0004h
(single 512KB bank, the configuration used on all known retail, devkit and
...
later board versions merged the SPU and CD-ROM into one chip that lacks the
additional chip selects and address lines required for this modification.<br/>
...
- Set ATTR to "Manual Write" (and wait until it is applied in STATX)<br/>
- Wait until Transfer Busy in STATX goes off (that, AFTER above apply-wait)<br/>
...
ATTR writes being applied in STATX won't work in that case (since ATTR was
already in manual write mode from previous block), so one must instead use some
hardcoded delay of at least 300h cycles; the BIOS is using a much longer bizarre
delay though).<br/>
...
#### SPU RAM DMA-Read (unstable reading, with DEV4\_CTRL.bit24-27 = zero)
Below describes some dirt effects and some trickery to get around those dirt
effects.<br/>
```
  Below problems (and workarounds) apply ONLY if DEV4\_CTRL.bit24-27 = zero.
  Ie. below info describes what happens when DEV4\_CTRL is mis-initialized.
  Normally one should set DEV4\_CTRL=220931E1h (and can ignore below info).
```
With DEV4\_CTRL.bit24-27=zero, reading SPU RAM via DMA works glitchy:<br/>
...
```
  15-0  Address in sound buffer divided by eight
```
See also: ATTR (IRQ enable/disable/acknowledge) and STATX (IRQ flag).<br/>
...
doesn't seem to trigger always (unknown why, but it seems to occasionally miss
...
(44100Hz/200h).<br/>
CD-Audio capture is always active (even CD-Audio output is disabled in ATTR,
...
#### Note
SPU interrupts are most frequently used for SPU-ADPCM streaming (overwriting
chunks of an endlessly looping sample in-place) and somewhat less commonly to
synchronize CD audio capture. Games that rely on SPU interrupts include (not
exhaustive):<br/>
- Metal Gear Solid (dialogue streaming)
- Dance Dance Revolution from 3rdMIX onwards (music streaming during gameplay)
- Um Jammer Lammy (CD capture + reverb buffer streaming, for audio effects)
- Legend of Mana
- Hercules (lip sync in the memory card loading screen)
- Tokimeki Memorial 2
- Crash Team Racing (lip sync, requires capture buffers)
- The Misadventures of Tron Bonne (dialogue streaming)
- Need For Speed 3
...
reverb buffer (the end address is fixed, at 7FFFEh). Writing a value to ESA
does additionally set the current buffer address to that value.<br/>

#### `0x1f801d98`: `EON0` (voice 0..15 echo/reverb enable)
```
  0-15  Voice 0..15 Destination (0=To Mixer, 1=To Mixer and to Reverb)
```

#### `0x1f801d9a`: `EON1` (voice 16..23 echo/reverb enable)
```
  0-7   Voice 16..23 Destination (0=To Mixer, 1=To Mixer and to Reverb)
  8-15  Not used
```
...
#### Reverb Bits in ATTR Register (R/W)
The ATTR register contains a Reverb Master Enable flag, and Reverb Enable flags
for External Audio input and CD Audio input.<br/>
...
```
  ...
  LeftOutput  = Lout*EVOLL
  RightOutput = Rout*EVOLR
...
within ESA..7FFFEh when exceeding that region.<br/>
...
#### Reverb Disable
ATTR.bit7 disables writes to reverb buffer, but reads from reverb buffer do
...
ATTR, and to zerofill the Reverb Buffer (so that no garbage values are output
...
1F801D84h, 1F801D86h, 1F801D98h, and the ATTR reverb bits should be set to
zero.<br/>
...
#### `0x1f801da0` - unused?
#### `0x1f801dbc` - 4 bytes - unused?
#### `0x1f801e60` - 32 bytes - unused?
The registers are read/write-able, although writing any values to them doesn't
seem to have any effect on sound output. Also, the SPU doesn't seem to modify
the registers at any time during sound output, nor reverb calculations, nor
activated external audio input... the registers seem to be just some kind of
general-purpose RAM.<br/>
